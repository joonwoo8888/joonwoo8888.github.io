...
Rook 은 오픈소스 클라우드 네이티브 스토리지 오케스트레이터로 , 클라우드 네이티브 환경과 기본적으로 통합 할 수있는 다양한 스토리지 솔루션 세트에 대한 플랫폼, 프레임 워크 및 지원한다.

High-level Storage Provider는 Ceph와 EdgeFS를 제공하며 그외 Cassandra, cockroachDB, Minio, NFS, YugabyteDB 를 지원하는거 같다.
...
Ceph 와 EdgeFS를 비교했을때 EdgeFS는 대용량 스토리지가 필요할때 적합한거같고, Ceph는 공유/확장에 적합한거같다.  우리는 3개의 서버를 HA Cluster로 묶을 예정이고 각 POD는 공유되는 파일 시스템이 필요하다.

![/assets/img/Kubernetes%20ROOK/Untitled%201.png](/assets/img/Kubernetes%20ROOK/Untitled%201.png)

### Ceph는 Block, Object, Shared의 유형의 스토리지를 제공한다.
...
우리가 필요한 Ceph의 Shared Stroage를 선택한 이유이다.
...
설치가 많이 심플하다.. git에 example로 클러스터 구성하는 yaml이 어느 정도 제공을하고 있다.

common.yaml, operator.yaml 은 공통적으로 실행하는 매니패스트다. cluster-test.yaml 은 클러스터를 구성하는 거에 따라 다른 yaml을 실행하면 된다. 

나는 테스트노드1에 먼저 설치를 하기 때문에 cluster-test.yaml을 실행하겠다.
...
- clister-minimal.yaml
    ...
    ceph-mon, ceph-mgr이 하나만 있는 최소의 클러스터 설정이다.
...
- taint가 설정되어있는지 확인해보아야 한다.

**Shared File System 시작이다..**

지금 까지 Rook, Ceph 를 설치 하였다. 이제 본격적으로 설치해야되는게 Shared File System이다. 그리고 POD에 퍼시스턴트 볼륨을 마운트하여 데이터가 영구적으로 남는지 확인을 해야한다.
...
이것도 엄청 심플하다... 실행하면 아래 처럼 CephFileSystem이 생성된다.

filesystem-test.yaml 옵션은 링크에 접속해서 확인하기 바란다.

[https://rook.io/docs/rook/v1.2/ceph-filesystem-crd.html](https://rook.io/docs/rook/v1.2/ceph-filesystem-crd.html)

이것으로 Shared File System 설치는 끝났다.. 하지만 쿠버네티스에 연결할려면 드라이브 처럼 storageclass 가 필요한거같다..
...
PersistentVolumeClaim을 1Gi Request로 생성하고 name은 cephfs-pvc로 만든다.

docker registry pod를 생성하는데 cephfs-pvc볼륨을 연결하고 /var/lib/registry경로에 마운트한다.

그리고 replaceas를 3개로 뛰어 이미지 저장이 공유되는지 확인한다.

또는 마운트된 경로에 파일을 추가하여 POD간에 공유가 되는지 확인한다.
